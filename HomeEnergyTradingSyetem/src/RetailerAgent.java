...
public class RetailerAgent extends Agent implements RetailerAgentInterface {
	...
	public void SetEnergyRate(double aEnergyRate) {
		...
		System.out.println("Agent " +getLocalName() +": energy rate is " +Double.toString(_energyRate));
	}
    ...
    protected void takeDown() 
    {
       try {
	    	 //get all registered client agents
	   		DFAgentDescription[] clientDescripts = getService("CLIENT");
	...
	private class RequestResponder extends SimpleAchieveREResponder {
		...
		public void Action() {
	    	...
	    	{
	    		cost = (energy * _energyRate) + (_random.nextInt(20) + 1);
	...
	private class CounterResponder extends SimpleAchieveREResponder {
		...
		protected ACLMessage prepareResponse(ACLMessage request) throws NotUnderstoodException, RefuseException {
			//registerHandleRequest(new Request());
			System.out.println("Agent "+getLocalName()+": COUNTER received from "+request.getSender().getName()+". Action is "+request.getContent());
	...
	private class ConfirmResponder extends SimpleAchieveREResponder {
		...
		protected ACLMessage prepareResponse(ACLMessage request) throws NotUnderstoodException, RefuseException {
			//registerHandleRequest(new Request());
			System.out.println("Agent "+getLocalName()+": CONFIRM received from "+request.getSender().getName()+". Action is "+request.getContent());
